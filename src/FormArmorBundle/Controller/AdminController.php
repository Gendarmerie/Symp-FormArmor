...
class AdminController extends Controller
{
	...
	// Gestion des statuts
	public function listeStatutAction($page, Request $request)
	{
    ...
    {
      return $this->redirectToRoute('form_armor_admin');
    }
	}
    ...
    public function listePlanFormationAction($page)
    {
      if ($page < 1)
  		...
  		$lesPlansFormations = $rep->listePlansFormation($page, $nbParPage);

  		// On calcule le nombre total de pages grâce au count($lesSessions) qui retourne le nombre total de sessions
  		$nbPages = ceil(count($lesPlansFormations) / $nbParPage);
  		...
  		// On donne toutes les informations nécessaires à la vue
  		return $this->render('FormArmorBundle:Admin:planFormation.html.twig', array(
  		  'lesPlansFormations' => $lesPlansFormations,
  		  'nbPages'     => $nbPages,
  		  'page'        => $page,
  		));
    }

    public function modifPlanFormationAction($id, Request $request) // Affichage du formulaire de modification d'une session
      {
          // Récupération de la formation d'identifiant $id
  		$em = $this->getDoctrine()->getManager();
  		...
  		// Création du formulaire à partir de la session "récupérée"
  		$form   = $this->get('form.factory')->create(PlanFormationType::class, $session);
  		...
  		if ($request->getMethod() == 'POST')
  		{
  			...
  			if ($form->isValid())
  			{
  				...
  				$nbParPage = $this->container->getParameter('nb_par_page');
  				// On récupère l'objet Paginator
  				$lesPlansFormations = $rep->listePlansFormation(1, $nbParPage);
  		...
  		// Si formulaire pas encore soumis ou pas valide (affichage du formulaire)
  		return $this->render('FormArmorBundle:Admin:formPlanFormation.html.twig', array('form' => $form->createView(), 'action' => 'modification'));
      }
  	public function suppPlanFormationAction($id, Request $request) // Affichage du formulaire de suppression d'une session
      {
          // Récupération de la session d'identifiant $id
  		$em = $this->getDoctrine()->getManager();
  		$rep = $em->getRepository('FormArmorBundle:Plan_formation');
  		...
  		// Si formulaire pas encore soumis ou pas valide (affichage du formulaire)
  		return $this->render('FormArmorBundle:Admin:formPlanFormation.html.twig', array('form' => $form->createView(), 'action' => 'SUPPRESSION'));
      }
